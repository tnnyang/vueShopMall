...
export default {
  install( Vue ){
    ...
    Vue.prototype.createElement = function () {
      ...
      str+= '<div class="crop_loading"><div class="crop_content"><img src="../static/images/loading.gif"><div class="crop_text">图片上传中</div></div></div>';
    ...
    //图片上传   可在此方法中通过接口将imageData上传到数据库
    Vue.prototype.postImg = function( imageData ) {    //imageData即为图片的路径
      //这边写图片的上传
      let self = this;
      self.destoried();
    ...
    Vue.prototype.compress = function ( img , Orientation ) {
      ...
      //进行最小压缩
      let ndata = canvas.toDataURL( 'image/jpeg' , 1);    //canvas.toDataURL(type, encoderOptions);   type：图片格式，默认为 image/png，encoderOptions：在指定图片格式为 image/jpeg 或 image/webp的情况下，可以从 0 到 1 的区间内选择图片的质量。如果超出取值范围，将会使用默认值 0.92。
